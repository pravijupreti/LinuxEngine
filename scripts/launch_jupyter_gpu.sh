...
monitor_browser_window() {
    ...
    # PORTABLE FIX: Get the directory where THIS script is located
    # This works on any PC regardless of installation path
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    
    # The git script is in the same directory as this launch script
    GIT_SCRIPT="$SCRIPT_DIR/git_auto_push.sh"
    
    # The workspace is where we are currently (your notebook directory)
    WORKSPACE="$(pwd)"
    
    # Call the git push script
    if [ -f "$GIT_SCRIPT" ]; then
        echo "Found git_auto_push.sh in $SCRIPT_DIR"
        chmod +x "$GIT_SCRIPT"
        # Run the git script from the workspace directory
        (cd "$WORKSPACE" && "$GIT_SCRIPT" "window_closed")
    else
        echo "❌ git_auto_push.sh not found in $SCRIPT_DIR!"
        echo "   Please ensure git_auto_push.sh is in the same directory as this script."
